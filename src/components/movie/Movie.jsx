...
function Movie(props) {
  ...
  const { userFavoritesData, setUserFavoritesData } =
    useContext(UserFavoritesContext);
  const isFavorite = userFavoritesData?.some((e) => e?.id === movieData?.id);
  ...
  function addFavorites(e) {
    ...
    setUserFavoritesData((prev) =>
      prev?.some((favorite) => favorite?.id === movieData?.id)
        ? prev?.filter((favorite) => favorite?.id !== movieData?.id)
        : [...prev, { id: movieData?.id, type: "movie" }],
    );
  }
  ...
  return (
    <div className="relative">
      <div
        ...
        style={{
          backgroundImage: `url(${HERO_IMG_URL}${movieData?.backdrop_path})`,
        }}
      >
          ...
          <div>
            ...
            <div className="flex items-center gap-x-2 my-7">
              ...
              <button
                ...
                className="bg-[gray]/50 border border-slate-700 py-1 px-4 rounded-sm flex items-center gap-x-1 text-xl text-white capitalize cursor-pointer hover:scale-105 transition-all duration-300"
              >
                {isFavorite ? <FaBookmark /> : <IoBookmarkOutline />}favorites
              </button>
            </div>
